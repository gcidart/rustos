...
pub fn time() -> Duration {
    let mut ecode: u64;
    let mut curr_sec: u64;
    let mut curr_subsec: u64;
    unsafe {
        asm!("svc $3
              mov $0, x0
              mov $1, x1
              mov $2, x7"
             : "=r"(curr_sec), "=r"(curr_subsec), "=r"(ecode)
             : "i"(NR_TIME)
             : "x0", "x1", "x7"
             : "volatile");
    }

    Duration::new(curr_sec, curr_subsec as u32)
...
pub fn write_str(msg: &str) {
    ...
    unsafe {
        asm!("mov x0, $2
              ...
              mov $1, x7"
              : "=r"(rmsg_len), "=r"(ecode)
              : "r"(msg_addr), "r"(msg_len), "i"(NR_WRITE_STR)
              : "x0", "x7"
              : "volatile");
    }
}
