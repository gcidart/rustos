...
impl VMManager {
    ...
    pub fn initialize(&self) {
        *self.kern_pt.lock() = Some(KernPageTable::new());
        let kern_page_table = self.kern_pt.lock();
        let kern_pt_addr = kern_page_table.as_ref().unwrap().get_baddr().as_usize();
    ...
    pub fn wait(&self) {
        ...
        loop {
            if self.ready_core_cnt.load(Ordering::Relaxed) == pi::common::NCORES {
                break;
            }
        }
        unsafe {
